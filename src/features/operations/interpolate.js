...
module.exports = function interpolate(sketch, pcurve, pt_a, pt_b, positions) {
  ...
  const oriented = pcurve
    .transform(flatten.matrix().rotate(angle).translate(...pt_a));
  ...
  return sketch.add(sketch.create({ entities: [new_pcurve] }));
};
