...
export default {
  ...
  computed: {
    ...
    errors() {
      return this.$store.getters.errors[this.$store.state.currentSketch];
    },
    underlines() {
  ...
  methods: {
    ...
    underline(markup) {
      const lines = markup.split(/\r?\n/).map((line, i) => {
        ...
        const hightlight = this.underlines[i] || '';
        return `<span data-line="${i}" class="line ${hightlight}">${line}</span>`;
      });
      return lines.join('\n');
    },
    highlighter(code) {
